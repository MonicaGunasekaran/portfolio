...
export default function MyCreations() {
  ...
  return (
    <section id="projects" className="py-16 bg-bodyColor-light/50 dark:bg-bodyColor-dark/50">
      <h2 className="text-4xl font-bold text-designColor-light dark:text-designColor-dark text-center mb-10">🎨 My Creations 💻</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-6">
        {projects.map((project, index) => (
          <div key={index} className="bg-bodyColor-light dark:bg-bodyColor-dark rounded-2xl shadow-md p-6 flex flex-col items-center text-center transition-transform duration-300 hover:scale-105">
            <div className="w-16 h-16 bg-bodyColor-light/80 dark:bg-bodyColor-dark/80 rounded-full flex justify-center items-center mb-4">
              <img src={project.logo} alt={project.name} className="w-10 h-10" />
            </div>
            <h3 className="text-xl font-semibold text-fontColor-light dark:text-fontColor-dark">{project.name}</h3>
            <p className="text-fontColor-light/80 dark:text-fontColor-dark/80 mt-2">{project.description}</p>
          </div>
        ))}
      </div>
    </section>
